...
save(mean_rqs_ea, file = 'mean_rqs_ea.Rda')
```

```{r echo=F}
load('mean_rqs_ea.Rda')
```
...
save(mean_rqs_sccs, file = 'mean_rqs_sccs.Rda')

````

```{r echo=F}
load('mean_rqs_sccs.Rda')
```
                        ...
                        merge(var_matcher) %>% # add titles
                          arrange(-ea) %>% # sick hack: order "title" for r sq from ea!
                            mutate(title = factor(title, title)) %>% 
                              melt(id = c('var_id','title'), variable.name = 'dataset')# melt for nice 
```
